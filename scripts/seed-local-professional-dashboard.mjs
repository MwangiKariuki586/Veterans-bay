...
try {
  ...
  // Seed additional future schedules for smooth drag testing (covers next 30 days)
  const futureSchedules = [
    [8, 7, 10, "Plumbing Repair"], [9, 8, 14, "Electrical Fix"], [10, 10, 9, "Deep Cleaning"], [11, 12, 11, "Appliance Service"], [12, 14, 15, "Pipe Replacement"], [13, 15, 10, "Wiring Check"], [14, 17, 13, "Kitchen Plumbing"], [15, 21, 10, "Bathroom Install"], [16, 22, 14, "AC Servicing"], [17, 24, 11, "Security Lighting"], [18, 28, 9, "Furniture Assembly"], [19, 30, 16, "Drain Unblocking"],
  ];
  for (const [idx, days, hour, serviceName] of futureSchedules) {
    const bid = `d6100000-0000-4000-8000-${String(idx).padStart(12, "0")}`;
    const jid = `d7100000-0000-4000-8000-${String(idx).padStart(12, "0")}`;
    const starts = at(days, hour);
    const ends = new Date(starts.getTime() + 90 * 60 * 1000);
    await client.query(`insert into bookings (id,professional_service_id,organisation_id,client_account_id,created_by_account_id,assigned_membership_id,origin,status,currency,total_minor,deposit_minor,expected_duration_minutes,starts_at,ends_at,timezone,cancellation_acknowledged_at,scope,exclusions,warranty_terms,payment_terms,completed_at) values ($1,$2,$3,$4,$5,$6,'PROFESSIONAL_CUSTOMER','CONFIRMED','KES',380000,0,90,$7,$8,'Africa/Nairobi',now(),'Future seeded schedule for drag testing.','None','Thirty-day warranty.','Manual.',null) on conflict (id) do update set starts_at=excluded.starts_at,ends_at=excluded.ends_at,status='CONFIRMED'`, [bid, ids.service, organisationId, ids.client, ownerId, ids.membership, starts, ends]);
    await client.query(`insert into jobs (id,booking_id,organisation_id,client_account_id,created_by_account_id,status,service_name,scope_snapshot,exclusions_snapshot,warranty_terms_snapshot,payment_terms_snapshot,currency,base_total_minor,approved_variation_total_minor,total_minor,scheduled_starts_at,scheduled_ends_at,checked_in_at,started_at,awaiting_confirmation_at,completed_at) values ($1,$2,$3,$4,$5,'TEAM_ASSIGNED',$6,'Future schedule.','None','Warranty.','Manual.','KES',380000,0,380000,$7,$8,null,null,null,null) on conflict (id) do update set status='TEAM_ASSIGNED',scheduled_starts_at=excluded.scheduled_starts_at,scheduled_ends_at=excluded.scheduled_ends_at`, [jid, bid, organisationId, ids.client, ownerId, serviceName, starts, ends]);
    await client.query(`insert into job_assignments (job_id,organisation_id,membership_id,assigned_by_account_id,active,reason) values ($1,$2,$3,$4,true,'Future schedule assignment.') on conflict do nothing`, [jid, organisationId, ids.membership, ownerId]);
    await client.query(`insert into booking_reservations (id,booking_id,organisation_id,membership_id,starts_at,ends_at,status) values ($1,$2,$3,$4,$5,$6,'ACTIVE') on conflict (booking_id) do update set starts_at=excluded.starts_at,ends_at=excluded.ends_at,status='ACTIVE'`, [`d8100000-0000-4000-8000-${String(idx).padStart(12,"0")}`, bid, organisationId, ids.membership, starts, ends]);
  }
