...
try {
  ...
  for (let i=0;i<3;i++) {
    await client.query(`insert into quotations (id,request_id,organisation_id,client_account_id,created_by_account_id,status,current_version_number) values ($1,$2,$3,$4,$5,'SUBMITTED',2) on conflict (id) do update set status='SUBMITTED',current_version_number=2,updated_at=now()`, [ids.quotations[i], ids.requests[i+2], organisationId, ids.client, ownerId]);
    await client.query(`insert into quotation_versions (id,quotation_id,version_number,status,currency,labour_minor,materials_minor,transport_minor,additional_charges_minor,subtotal_minor,discount_minor,tax_minor,total_minor,deposit_minor,expected_duration_minutes,valid_until,scope,exclusions,warranty_terms,payment_terms,created_by_account_id,submitted_at) values ($1,$2,1,'SUBMITTED','KES',275000,150000,50000,0,475000,0,0,475000,0,120,'2025-12-31T17:00:00.000Z','Initial synthetic local plumbing scope.','Unrelated fixtures excluded.','Thirty-day workmanship warranty.','Payment is recorded manually.',$3,'2025-01-15T09:00:00.000Z') on conflict (id) do nothing`, [ids.historicalQuoteVersions[i], ids.quotations[i], ownerId]);
    await client.query(`insert into quotation_versions (id,quotation_id,version_number,status,currency,labour_minor,materials_minor,transport_minor,additional_charges_minor,subtotal_minor,discount_minor,tax_minor,total_minor,deposit_minor,expected_duration_minutes,valid_until,scope,exclusions,warranty_terms,payment_terms,created_by_account_id,submitted_at) values ($1,$2,2,'SUBMITTED','KES',300000,150000,50000,0,500000,0,0,500000,0,120,$3,'Synthetic local plumbing scope.','Unrelated fixtures excluded.','Thirty-day workmanship warranty.','Payment is recorded manually.',$4,now()) on conflict (id) do nothing`, [ids.quoteVersions[i], ids.quotations[i], i===2 ? new Date('2025-12-31T17:00:00.000Z') : new Date('2035-12-31T17:00:00.000Z'), ownerId]);
  }
