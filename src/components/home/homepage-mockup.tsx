  ...
  return (
    <section className="relative overflow-hidden rounded-[22px] border border-black/8 bg-white lg:col-start-1 lg:row-start-1 lg:row-span-3" aria-labelledby="hero-title">
      ...
      <div className="relative z-10 flex h-full min-h-[650px] w-full flex-col items-start bg-white/72 px-7 py-8 sm:min-h-[570px] lg:min-h-0 lg:w-[54%] lg:bg-transparent">
        ...
        <h1 id="hero-title" className="mt-9 text-[2.65rem] leading-[1.04] font-bold tracking-title sm:text-[3.55rem]">Find Trusted<br />Home Service<br />Professionals.</h1>
...
function PopularServices() {
  return (
    <section className="rounded-[22px] border border-black/8 bg-white p-5 lg:col-start-2 lg:row-start-1" aria-labelledby="popular-heading">
      <div className="flex items-center justify-between"><h2 id="popular-heading" className="text-[1rem] font-semibold">Popular Services</h2><Link href="/marketplace" className="type-caption text-muted-foreground">View all</Link></div>
      <div className="mt-5 grid grid-cols-5 gap-2">
        {popularServices.map(({ label, icon: Icon, className }) => (
          <Link key={label} href={`/marketplace?category=${encodeURIComponent(label)}`} className="grid justify-items-center gap-2 text-center type-caption font-medium">
  ...
  return (
    <Link href="/marketplace" className="relative overflow-hidden rounded-[22px] border border-black/8 bg-white p-5 lg:col-start-2 lg:row-start-2">
      ...
      <div className="relative z-10 max-w-[58%]">
        <span className="inline-flex items-center gap-1.5 rounded-full bg-primary/20 px-2.5 py-1.5 type-caption font-semibold text-[#718f13]"><Star className="size-3 fill-current" /> Featured Pro</span>
...
function CategoryPanel() {
  return (
    <section className="rounded-[22px] border border-black/8 bg-white p-4.5" aria-labelledby="categories-heading">
      <div className="flex items-center justify-between"><h2 id="categories-heading" className="text-[0.95rem] font-semibold">Popular Categories</h2><Link href="/marketplace" className="type-caption text-muted-foreground">View all</Link></div>
      <div className="mt-5 grid grid-cols-4 gap-3">
        {categoryCards.map((category) => <Link key={category.label} href={`/marketplace?category=${category.label.toLowerCase()}`} className="type-caption font-medium"><Image src={category.image} alt="" width={130} height={105} className="aspect-[0.9] w-full rounded-[15px] object-cover" /><span className="mt-2.5 block text-center">{category.label}</span></Link>)}
      </div>
    </section>
  );
}
...
function BookingStats() {
  return (
    <section className="rounded-[22px] border border-black/5 bg-[#e9f5f5] p-5">
      <div className="flex -space-x-2">{[0, 1, 2, 3].map((item) => <Image key={item} src="/images/header-avatar.png" alt="" width={32} height={32} className="size-8 rounded-full border-2 border-white object-cover" />)}<span className="grid size-8 place-items-center rounded-full border-2 border-white bg-white type-caption font-semibold text-trust">+1K</span></div>
      <p className="mt-5 text-[1.95rem] font-semibold tracking-title text-[#165365]">15k+</p>
      <p className="mt-1 text-[0.8rem] font-semibold">Bookings Completed</p>
      <div className="mt-4 flex items-center gap-2 type-caption"><ShieldCheck className="size-4 fill-primary text-[#5f8d11]" /> 4.8 <span className="text-muted-foreground">| 2,350 reviews</span></div>
      <p className="mt-3.5 flex items-center gap-2 type-caption"><ShieldCheck className="size-4 text-trust" /> Verified. Rated. Trusted.</p>
    </section>
  );
}
...
function FastBooking() {
  return (
    <section className="relative overflow-hidden rounded-[22px] border border-black/8 bg-white p-5">
      ...
      <div className="relative z-10 max-w-[55%]"><p className="type-caption text-muted-foreground">Why Veterans Bay?</p><h2 className="mt-5 text-[1.12rem] font-semibold leading-tight">Fast Booking,<br />Clear Quotes.</h2><p className="mt-2.5 type-caption leading-4 text-muted-foreground">Transparent pricing and<br />real-time availability.</p><p className="mt-7 inline-flex items-center gap-2 rounded-full bg-white px-4 py-2 text-[0.72rem] font-semibold shadow-[0_7px_18px_rgba(10,24,35,0.1)]"><Star className="size-3.5 fill-[#ffb81c] text-[#ffb81c]" /> 4.9</p></div>
    </section>
  );
}
