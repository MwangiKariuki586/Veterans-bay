    ...
    <div className="space-y-8">
        ...
        <div className="grid gap-6 bg-[linear-gradient(135deg,#071522_0%,#132a3a_62%,#425f29_100%)] p-6 text-white sm:p-9 lg:grid-cols-[auto_minmax(0,1fr)_auto] lg:items-center">
          {profile.logoUrl ? (
            <Image
              ...
              width={104}
              height={104}
              className="size-24 rounded-[26px] border border-white/15 bg-white object-cover"
            />
          ) : (
            <span className="grid size-24 place-items-center rounded-[26px] bg-[#b8f52a] text-3xl font-bold text-[#071522]">
          ...
          <div>
            <div className="flex flex-wrap gap-2">
              {profile.verified ? (
                <span className="inline-flex items-center gap-1.5 rounded-full bg-[#b8f52a] px-3 py-1 text-xs font-semibold text-[#071522]">
                  <BadgeCheck className="size-4" /> Verified professional
                </span>
              ) : null}
            ...
            <h1 className="mt-4 text-4xl font-bold tracking-title sm:text-5xl">
              {profile.businessName}
            </h1>
            <p className="mt-4 max-w-3xl text-sm leading-7 text-white/75">
              {profile.description ??
                "This professional has not added a public introduction yet."}
            </p>
          </div>
          <div className="grid gap-2 text-sm text-white/80">
            {profile.operatingLocation ? (
              <span className="inline-flex items-center gap-2">
                <MapPin className="size-4 text-[#b8f52a]" />{" "}
                {profile.operatingLocation}
              </span>
            ) : null}
            {profile.availabilitySummary ? (
              <span className="inline-flex items-center gap-2">
                <Clock3 className="size-4 text-[#b8f52a]" />{" "}
                {profile.availabilitySummary}
              </span>
            ) : null}
          </div>
      ...
      <div className="grid gap-4 sm:grid-cols-3">
        <Metric
          ...
          value={profile.rating == null ? "New" : profile.rating.toFixed(1)}
          description={
        ...
        <Metric
          ...
          description="Recorded through Veterans Bay"
        />
        <Metric
          label="Response"
          value={profile.responseIndicator ?? "New"}
          description={
        ...
        {profile.services.length > 0 ? (
          <div className="mt-5 grid gap-5 md:grid-cols-2 xl:grid-cols-3">
            {profile.services.map((service) => (
              <PublicServiceCardView key={service.slug} service={service} />
            ))}
          </div>
        ) : (
          <StatePanel
      ...
      <section>
        ...
        {profile.reviews && profile.reviews.length > 0 ? (
          ...
          <StatePanel
            ...
            description="Verified feedback from completed Veterans Bay jobs will be published here."
            className="mt-5"
          />
        )}
      </section>
      ...
      <section>
        ...
        <h2 className="mt-2 text-2xl font-bold tracking-title">Recent work</h2>
        {profile.portfolio.length > 0 ? (
          ...
          <StatePanel
            ...
            description="No public portfolio work has been added yet."
            className="mt-5"
          />
        )}
      </section>
...
function Metric({
  ...
  return (
    <Surface className="p-5 shadow-none">
      <p className="text-xs font-semibold uppercase tracking-[0.16em] text-[#68717b]">
        {label}
      </p>
      <p className="mt-3 text-2xl font-bold">{value}</p>
      <p className="mt-1 text-xs text-[#68717b]">{description}</p>
    </Surface>
  );
}
  ...
  return (
    <Surface className="overflow-hidden p-0 shadow-none">
      {service.imageUrl ? (
        ...
        <div className="grid aspect-[16/9] place-items-center bg-[#eef8c8]">
          <Wrench className="size-8 text-[#5f8d11]" />
        </div>
      )}
      <div className="p-5">
        <div className="flex items-center justify-between gap-3">
          ...
          <span className="text-sm font-bold">{formatPrice(service)}</span>
        </div>
        <h3 className="mt-3 text-lg font-bold">{service.name}</h3>
        <p className="mt-2 line-clamp-2 text-sm leading-6 text-[#68717b]">
          {service.description}
        </p>
        <Link
