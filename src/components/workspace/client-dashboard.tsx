...
function MetricCard({
  ...
  const tones: Record<string, string> = {
    ...
    green: "bg-[#eaf5e5] text-[#2f7d18]",
    orange: "bg-[#fff1df] text-[#d9730d]",
    yellow: "bg-[#fff7e8] text-[#b77900]",
  };
  const displayValue =
    typeof value === "number" ? value.toLocaleString() : value;
  return (
    <Surface className="flex h-[128px] flex-col rounded-[16px] p-3 shadow-[0_4px_16px_rgba(15,31,43,0.04)]">
      <div className="grid grid-cols-[36px_minmax(0,1fr)] gap-x-2.5">
        ...
        <span className="min-w-0">
          <span className="line-clamp-2 min-h-7 text-[0.72rem] font-medium leading-3.5 text-muted-foreground">
            {label}
          </span>
          <span className="mt-0.5 block type-metric font-semibold leading-none text-foreground">
            {displayValue}
          </span>
          <span
            className={cn(
              "mt-0.5 block truncate text-[0.67rem] leading-none",
              hintTone === "danger"
      ...
      <Link
        href={href}
        className="ml-[46px] mt-1.5 inline-flex w-fit items-center gap-1 py-0.5 text-[0.7rem] font-semibold text-trust underline-offset-4 transition-colors hover:text-foreground hover:underline"
      >
        {action}
